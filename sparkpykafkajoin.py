...
spark = SparkSession.builder.appName("stedi-kafka-join").getOrCreate()
...
redisRawDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("subscribe","redis-server").option("startingOffsets","earliest").load()

# TO-DO: cast the value column in the streaming dataframe as a STRING 
streamingDF = redisRawDF.selectExpr('cast(key as string) key','cast(value as string) value')
...
streamingDF.withColumn('value', from_json('value', redisMessageSchema)).select(col('value.*')).createOrReplaceTempView("RedisSortedSet")  
...
encodedCustDF = spark.sql("select zSetEntries[0].element as encodedCustomer from RedisSortedSet")   
...
unbasedCustomerDF = encodedCustDF.withColumn('encodedCustomer', unbase64(encodedCustDF.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
unbasedCustomerDF.withColumn('encodedCustomer', from_json('encodedCustomer', customerMessageSchema)).select(col('encodedCustomer.*')).createOrReplaceTempView("CustomerRecords")
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
stediRawDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("subscribe","stedi-events").option("startingOffsets", "earliest").load()
                                   
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediStreamingDF = stediRawDF.selectExpr('cast(key as string) key','cast(value as string) value')
...
stediStreamingDF.withColumn('value', from_json('value', scoreSchema)).select(col('value.*')).createOrReplaceTempView("CustomerRisk")
...
finalDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("customer = email"))
...
#finalDF.writeStream.outputMode("append").format("console").start().awaitTermination()
finalDF.select("customer","score","email","birthYear").selectExpr("cast(customer as string) key","to_json(struct(*)) as value").writeStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("topic","stedi-graph").option("checkpointLocation","/tmp/localcheckpointLocatin").start().awaitTermination()
