...
spark = SparkSession.builder.appName("stedi-graph-update").getOrCreate()
...
redisDF = redisRawDF.selectExpr('cast(key as string) key', 'cast(value as string) value')
...
redisDF.withColumn('value', from_json('value', redisMessageSchema)).select(col('value.*')).createOrReplaceTempView("RedisSortedSet")            
